<nav x-data="{ open: false }" class="bg-gray-800/50 backdrop-blur-xl border-b border-gray-700/50 shadow-2xl relative z-50">
            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if(auth()->user()->is_admin)
                        <x-nav-link :href="route('admin.dashboard')" :active="request()->routeIs('admin.*')" class="text-white hover:text-red-400 transition-colors">
                            Admin Panel
                        </x-nav-link>
                    @endif
                </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-gray-800/95 backdrop-blur-xl border-t border-gray-700/50 relative z-50">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if(auth()->user()->is_admin)
                <x-responsive-nav-link :href="route('admin.dashboard')" :active="request()->routeIs('admin.*')" class="text-white hover:bg-red-400/50" @click="open = false">
                    Admin Panel
                </x-responsive-nav-link>
            @endif
        </div>
