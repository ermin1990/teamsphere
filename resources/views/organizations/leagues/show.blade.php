                                ...
                                <form method="POST" action="{{ route('leagues.update', $league) }}" class="space-y-3">
                                    ...
                                    <div class="flex items-center justify-between">
                                        ...
                                        <div class="relative">
                                            ...
                                            <div class="w-16 h-8 bg-gray-600 rounded-full relative cursor-pointer transition-colors duration-300 {{ $league->isPublic() ? 'bg-blue-600' : 'bg-gray-600' }}"
                                                 onclick="document.querySelector('input[name=is_public]').click();">
                                                
                                                <!-- Slider -->
                                                <div class="absolute top-1 w-6 h-6 bg-white rounded-full shadow-md transform transition-transform duration-300 {{ $league->isPublic() ? 'translate-x-8' : 'translate-x-1' }}">
                                                    <!-- Icon inside slider -->
                                                ...
                                                <div class="absolute inset-0 flex items-center justify-between px-2 text-xs font-medium">
                                                    <span class="text-gray-400 {{ $league->isPublic() ? 'opacity-0' : 'opacity-100' }} transition-opacity duration-300">OFF</span>
                                                    <span class="text-white {{ $league->isPublic() ? 'opacity-100' : 'opacity-0' }} transition-opacity duration-300">ON</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
