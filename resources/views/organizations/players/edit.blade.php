    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-gray-400 mt-1">{{ $player->name }} • {{ $organization->name }}</p>
            </div>
        </div>
    </x-slot>
            ...
            <form action="{{ route('players.update', $player) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="date"
                           id="date_of_birth"
                           name="date_of_birth"
                           value="{{ old('date_of_birth', $player->date_of_birth ? \Carbon\Carbon::parse($player->date_of_birth)->format('Y-m-d') : '') }}"
                ...
                <div>
                    <label class="flex items-center">
                        <input type="checkbox"
                               ...
                               class="rounded border-white/20 bg-white/10 text-purple-600 focus:ring-purple-500 focus:ring-2">
                        <span class="ml-3 text-sm font-medium text-white">
                            {{ __('Active Player') }}
                        </span>
                    </label>
                    <p class="mt-1 text-sm text-white/60">
                        {{ __('Inactive players will not appear in team selections and statistics.') }}
                    </p>
                </div>
