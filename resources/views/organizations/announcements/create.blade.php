        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-gray-800/50 backdrop-blur-xl rounded-2xl border border-gray-700/50 shadow-xl overflow-hidden">
                <div class="bg-gradient-to-r from-blue-600/20 to-purple-600/20 border-b border-gray-700/50 px-8 py-6">
                    <h1 class="text-2xl font-bold text-white">Nova Obavijest</h1>
                    <p class="text-gray-400 mt-1">{{ $organization->name }}</p>
                </div>

                <div class="p-8">
                    <form method="POST" action="{{ route('organizations.announcements.store', $organization) }}">
                        @csrf
                        ...
                        <div class="mb-6">
                            ...
                            <select name="competition_id" id="competition_id" class="w-full bg-gray-700/50 border border-gray-600/50 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
                                <option value="" {{ old('competition_id', $selectedCompetitionId) == null ? 'selected' : '' }}>Cijela organizacija (sve lige)</option>
                                @foreach($competitions as $comp)
                                    <option value="{{ $comp->id }}" {{ (int) old('competition_id', $selectedCompetitionId) === $comp->id ? 'selected' : '' }}>{{ $comp->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-6">
                            ...
                            <input type="text" name="title" id="title" value="{{ old('title') }}"
                                   ...
                                   placeholder="Naslov obavijesti" required>
                            @error('title')
                                <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="body" id="body" rows="8"
                                      class="w-full bg-gray-700/50 border border-gray-600/50 rounded-xl px-4 py-3 text-white placeholder-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
                                      placeholder="Tekst obavijesti..." required>{{ old('body') }}</textarea>
                            @error('body')
                                <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-6 flex items-start gap-3 bg-gray-700/30 border border-gray-600/50 rounded-xl p-4">
                            ...
                            <label for="is_featured" class="text-sm text-white">
                                Prikaži izdvojeno (istaknuto)
                                <span class="block text-gray-400 text-xs mt-1">Pojavljuje se u istaknutom prostoru odmah ispod naslova. Ako je izabrana konkretna liga, prikazuje se samo tamo. Ako je izabrana "Cijela organizacija", prikazuje se i na stranici organizacije i na svim ligama. Uvijek se prikazuje samo jedna izdvojena obavijest po stranici - najnovija koja ima ovo uključeno; isključi je bilo kad izmjenom ove obavijesti.</span>
                            </label>
                        </div>
