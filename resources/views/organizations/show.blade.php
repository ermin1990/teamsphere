<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
                ...
                <div class="bg-gray-800/50 backdrop-blur-xl rounded-2xl p-6 border border-gray-700/50 shadow-xl">
                    ...
                    <div class="mt-4">
                        <div class="flex items-center text-sm">
                            ...
                            <span class="text-gray-500 ml-2">{{ __('total players') }}</span>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-gray-800/50 backdrop-blur-xl rounded-2xl p-6 border border-gray-700/50 shadow-xl">
                ...
                @else
                    <div class="text-center py-8">
                        ...
                        <h4 class="text-white font-semibold mb-2">{{ __('No leagues yet') }}</h4>
                        <p class="text-gray-400 mb-4">{{ __('Create your first league to start organizing competitions') }}</p>
                        @if($organization->canCreateMoreLeagues())
                        ...
                        @else
                            <div class="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-4 inline-block">
                                <p class="text-yellow-400 text-sm">{{ __('You have reached the maximum number of leagues for this organization') }}</p>
                            </div>
                        @endif
                    </div>
                @endif
            ...
            <div class="bg-gray-800/50 backdrop-blur-xl rounded-2xl p-6 border border-gray-700/50 shadow-xl">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-xl font-bold text-white">{{ __('Players') }}</h3>
                                ...
                                <div class="bg-gray-700/30 rounded-xl p-4 hover:bg-gray-600/30 transition-all duration-200 transform hover:scale-[1.02] cursor-pointer">
                                    <div class="flex items-center space-x-3 mb-3">
                                        ...
                                        <div class="flex-1">
                                            ...
                                            <p class="text-gray-400 text-sm">{{ $player->position ?: __('No position') }}</p>
                                        </div>
                                    ...
                                    <div class="space-y-2">
                                        <div class="flex justify-between text-sm">
                                            <span class="text-gray-400">{{ __('Type') }}:</span>
                                            <span class="text-{{ $player->user ? 'green' : 'yellow' }}-400">{{ $player->user ? __('Registered') : __('Named') }}</span>
                                        </div>
                                        <div class="flex justify-between text-sm">
                                            <span class="text-gray-400">{{ __('Status') }}:</span>
                                            <span class="text-{{ $player->is_active ? 'green' : 'red' }}-400">{{ $player->is_active ? __('Active') : __('Inactive') }}</span>
                                        </div>
                                    </div>
                                </div>
                            </a>
                    ...
                    @if((optional($organization->players)->count() ?? 0) > 6)
                        <div class="mt-4 text-center">
                            <a href="{{ route('organizations.players.index', $organization) }}" class="text-emerald-400 hover:text-emerald-300 text-sm font-medium transition-colors">
                                {{ __('View all') }} {{ optional($organization->players)->count() ?? 0 }} {{ __('players') }} →
                            </a>
                        </div>
                    @endif
                @else
                    <div class="text-center py-8">
                        ...
                        <p class="text-gray-400 mb-4">{{ __('Add players to start building your teams and tracking statistics') }}</p>
            ...
            <div class="bg-gray-800/50 backdrop-blur-xl rounded-2xl p-6 border border-gray-700/50 shadow-xl">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-xl font-bold text-white">{{ __('Recent Friendly Matches') }}</h3>
                    <div class="flex items-center space-x-3">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <!-- Details -->
                <div class="bg-gray-800/50 backdrop-blur-xl rounded-2xl p-6 border border-gray-700/50 shadow-xl">
                    <h3 class="text-xl font-bold text-white mb-4">{{ __('Organization Details') }}</h3>
                    <div class="space-y-4">
                        ...
                        <div class="flex justify-between">
                            <span class="text-gray-400">{{ __('URL Slug') }}:</span>
                ...
                <!-- Plan & Limits -->
                <div class="bg-gray-800/50 backdrop-blur-xl rounded-2xl p-6 border border-gray-700/50 shadow-xl">
                    <h3 class="text-xl font-bold text-white mb-4">{{ __('Plan & Limits') }}</h3>
                    <div class="space-y-4">
                        <div class="flex justify-between">
                            <span class="text-gray-400">{{ __('Your Plan') }}:</span>
                            <span class="text-blue-400 font-medium">{{ $organization->user->currentPlan() ? $organization->user->currentPlan()->name : 'Free' }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-400">{{ __('Leagues Used') }}:</span>
                        ...
                        <div class="pt-2">
                            <p class="text-gray-400 text-sm">{{ __('Remaining leagues') }}: <span class="text-white">{{ $organization->getRemainingLeaguesCount() }}</span></p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-red-500/10 border border-red-500/20 rounded-2xl p-6">
                <h3 class="text-xl font-bold text-red-400 mb-4">{{ __('Danger Zone') }}</h3>
                <p class="text-gray-400 mb-4">{{ __('Once you delete this organization, there is no going back. Please be certain.') }}</p>
                <form method="POST" action="{{ route('organizations.destroy', $organization) }}" onsubmit="return confirm('{{ __('Are you sure you want to delete this organization? This action cannot be undone.') }}')">
                    @csrf
                    @method('DELETE')
