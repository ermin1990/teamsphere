...
<title>Zabilježi meč - MojTurnir</title>
...
<main class="lg:ml-[260px] lg:mt-16 p-margin-mobile lg:p-gutter min-h-screen">
    <div class="max-w-2xl mx-auto">
        <div class="mb-6 lg:mb-8">
            <h1 class="font-headline-lg-mobile lg:font-display text-headline-lg-mobile lg:text-headline-lg text-primary tracking-tight">Zabilježi meč</h1>
            <p class="text-on-surface-variant text-sm lg:text-base mt-1">{{ $competition->name }}</p>
        </div>
        ...
        <div class="bg-surface-container-low border border-outline-variant rounded-xl p-5 lg:p-8">
            ...
            <form method="POST" action="{{ route('player.matches.store', $competition) }}" class="space-y-6">
                ...
                <div>
                    <label for="opponent_id" class="block text-sm font-semibold mb-2 text-on-surface">Protivnik</label>
                ...
                <div>
                    <label for="played_at" class="block text-sm font-semibold mb-2 text-on-surface">Datum meča</label>
                    <input type="datetime-local" id="played_at" name="played_at" value="{{ now()->format('Y-m-d\TH:i') }}"
                           class="w-full px-4 py-3 rounded-xl text-sm bg-surface-container-highest border border-outline-variant text-on-surface focus:outline-none focus:border-primary transition-all">
                </div>

                <div>
                    <label class="block text-sm font-semibold mb-2 text-on-surface">Rezultat po setovima ({{ $unitLabel }})</label>
                    ...
                    <div class="space-y-2">
                        @for($i = 0; $i < $maxSets; $i++)
                            <div class="flex items-center gap-3">
                                <span class="text-sm w-14 font-medium text-on-surface-variant">Set {{ $i + 1 }}</span>
                                <input type="number" name="sets[{{ $i }}][mine]" min="0" placeholder="Ja"
                                       class="w-20 text-center px-3 py-2.5 rounded-xl bg-surface-container-highest border border-outline-variant text-on-surface focus:outline-none focus:border-primary transition-all">
                                <span class="text-on-surface-variant">:</span>
                                <input type="number" name="sets[{{ $i }}][theirs]" min="0" placeholder="Protivnik"
                                       class="w-24 text-center px-3 py-2.5 rounded-xl bg-surface-container-highest border border-outline-variant text-on-surface focus:outline-none focus:border-primary transition-all">
                            </div>
                        @endfor
                    </div>
                </div>
                ...
                <div class="flex gap-3 pt-2">
                    <a href="{{ route('player.dashboard.matches') }}" class="flex-1 text-center px-4 py-3 rounded-full font-semibold transition-all active:scale-95 bg-surface-container-highest text-on-surface-variant">
                        Odustani
                    </a>
                    <button type="submit" class="flex-1 px-4 py-3 font-semibold rounded-full transition-all active:scale-95 bg-primary text-on-primary">
                        Sačuvaj meč
                    </button>
                </div>
            </form>
