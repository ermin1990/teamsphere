<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-white leading-tight">
            {{ __('Dodaj Sto') }} - {{ $organization->name }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6">
                    ...
                    <form action="{{ route('organizations.tables.store', $organization) }}" method="POST" class="space-y-6">
                        @csrf
                        ...
                        <div>
                            ...
                            <textarea name="description" 
                                      ...
                                      rows="3"
                                      class="w-full bg-gray-700 border border-gray-600 text-white rounded-lg px-4 py-2 focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50"
                                      placeholder="Dodatne informacije o stolu (lokacija, karakteristike, itd.)">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
