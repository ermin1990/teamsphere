...
<title>Obavijesti - {{ $competition->name }} - {{ $organization->name }}</title>
...
<main class="lg:ml-[260px] lg:mt-16 mt-0 p-margin-mobile lg:p-gutter min-h-screen">
    <div class="max-w-container-max mx-auto">
        ...
        @include('public.leagues._tabs', ['activeTab' => 'announcements'])
                    ...
                    <div class="p-4 lg:p-6 bg-surface-container-low border border-outline-variant rounded-xl">
                        <div class="flex flex-wrap items-center gap-2 mb-2">
                            @if($announcement->isOrganizationWide())
                                <span class="bg-secondary-soft text-secondary px-2.5 py-0.5 rounded-full text-label-bold uppercase">Organizacija</span>
                            @endif
                            <span class="text-xs text-on-surface-variant">{{ $announcement->created_at->format('d.m.Y. H:i') }}</span>
                        </div>
        ...
        @else
            <div class="text-center py-16 bg-surface-container-low border border-outline-variant rounded-xl">
                ...
                <p class="text-on-surface-variant text-sm">Ovdje će se pojaviti obavijesti organizatora za ovo takmičenje.</p>
            </div>
        @endif
    </div>
</main>
