        ...
        <div class="space-y-6">
            ...
            <div class="bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        <label class="block text-sm font-medium text-gray-400">{{ __('Type') }}</label>
                        <div class="mt-1">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-400">{{ __('Status') }}</label>
                        <div class="mt-1">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-400">{{ __('Submitted By') }}</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-400">{{ __('Date Submitted') }}</label>
                        <div class="mt-1 text-white">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-2">{{ __('Subject') }}</label>
            ...
            <!-- Update Status Form -->
            <div class="bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20">
                <h3 class="text-lg font-semibold text-white mb-4">{{ __('Update Status') }}</h3>
                ...
                <form action="{{ route('admin.bug-reports.update', $bugReport) }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        <label for="status" class="block text-sm font-medium text-white mb-2">{{ __('Status') }}</label>
                        <select id="status" name="status" class="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                            <option value="pending" {{ $bugReport->status === 'pending' ? 'selected' : '' }}>{{ __('Pending') }}</option>
                            <option value="in_review" {{ $bugReport->status === 'in_review' ? 'selected' : '' }}>{{ __('In Review') }}</option>
                            <option value="resolved" {{ $bugReport->status === 'resolved' ? 'selected' : '' }}>{{ __('Resolved') }}</option>
                            <option value="closed" {{ $bugReport->status === 'closed' ? 'selected' : '' }}>{{ __('Closed') }}</option>
                        </select>
                    </div>

                    <div>
                        <label for="admin_notes" class="block text-sm font-medium text-white mb-2">{{ __('Admin Notes') }}</label>
                        <textarea id="admin_notes" name="admin_notes" rows="4" class="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent" placeholder="{{ __('Add internal notes about this report...') }}">{{ $bugReport->admin_notes }}</textarea>
                    </div>
