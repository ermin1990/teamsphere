...
<title>Registracija - MojTurnir</title>
...
<main class="relative z-10 w-full max-w-[440px] px-margin-mobile py-12 flex flex-col items-center">
    <header class="w-full text-center mb-8">
        ...
        <p class="font-headline-md text-headline-md text-on-surface-variant mt-1">Kreirajte nalog</p>
    </header>
    ...
    <section class="w-full bg-surface-container p-6 rounded-xl border border-outline-variant login-glow">
        ...
        <form method="POST" action="{{ route('register') }}" class="flex flex-col gap-5">
            ...
            <!-- Name -->
            <div class="space-y-2">
                <label for="name" class="font-label-bold text-label-bold text-on-surface-variant ml-1 uppercase">Puno ime</label>
                ...
                <x-input-error :messages="$errors->get('name')" class="text-error text-sm" />
            </div>
            ...
            <div class="space-y-2">
                ...
                <div class="grid grid-cols-3 gap-3">
                    ...
                    <label class="relative cursor-pointer">
                        <input type="radio" name="role" value="venue" class="peer sr-only" {{ old('role') === 'venue' ? 'checked' : '' }}>
                        <div class="px-3 py-3 bg-surface-container-lowest border border-outline-variant rounded-lg text-center text-on-surface-variant peer-checked:border-primary peer-checked:bg-primary/10 peer-checked:text-on-surface transition-all duration-200">
            ...
            <!-- Password -->
            <div class="space-y-2">
                <label for="password" class="font-label-bold text-label-bold text-on-surface-variant ml-1 uppercase">Lozinka</label>
                <div class="relative group">
            ...
            <!-- Confirm Password -->
            <div class="space-y-2">
                <label for="password_confirmation" class="font-label-bold text-label-bold text-on-surface-variant ml-1 uppercase">Potvrdite lozinku</label>
                <div class="relative group">
                    <span class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-on-surface-variant group-focus-within:text-primary transition-colors">lock</span>
                    <input id="password_confirmation" type="password" name="password_confirmation" required autocomplete="new-password"
                           placeholder="••••••••"
                           class="w-full h-14 pl-12 pr-12 bg-surface-container-lowest border border-outline-variant rounded-lg text-on-surface font-body-md focus:border-primary focus:ring-1 focus:ring-primary/30 transition-all outline-none">
                    <button type="button" id="toggle-password-confirmation" class="absolute right-4 top-1/2 -translate-y-1/2 text-on-surface-variant hover:text-on-surface">
                        <span class="material-symbols-outlined" id="toggle-password-confirmation-icon">visibility</span>
                    </button>
                </div>
                <x-input-error :messages="$errors->get('password_confirmation')" class="text-error text-sm" />
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        function wireToggle(buttonId, iconId, inputId) {
        ...
        wireToggle('toggle-password', 'toggle-password-icon', 'password');
